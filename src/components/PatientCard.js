...
export function PatientCard({ row }) {
    ...
    const daysNode = row.remaining_days !== null ? (row.remaining_days < 0 ? (<div className="flex items-center gap-1.5 text-red-700 dark:text-red-400 font-extrabold text-base mt-2">
        <AlertTriangle className="h-4 w-4"/>
        متأخر {Math.abs(row.remaining_days)} {Math.abs(row.remaining_days) === 1 ? "يوم" : "أيام"}
      </div>) : row.remaining_days <= 3 ? (<div className="flex items-center gap-1.5 text-amber-700 dark:text-amber-400 font-extrabold text-base mt-2">
        <Clock className="h-4 w-4"/>
        متبقي {row.remaining_days} {row.remaining_days === 1 ? "يوم" : "أيام"}
      </div>) : (<div className="flex items-center gap-1.5 text-foreground font-bold text-sm mt-2">
    ...
    return (<>
      <Card className={`p-3 border-r-4 hover:shadow-md transition ${borderTone(meta.key)}`}>
          ...
          <div className="flex items-start gap-3">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {row.is_shared && (<Badge variant="outline" className="text-[10px]">
        ...
        <div className="mt-3">
          {isPartial ? (<Button className="w-full h-11 bg-info text-info-foreground hover:bg-info/90" onClick={(e) => { stop(e); setRemOpen(true); }}>
              <Clock className="h-4 w-4 ml-2"/> صرف متبقي
            </Button>) : (<Button className="w-full h-11" onClick={(e) => { stop(e); setDispOpen(true); }}>
              <CheckCircle2 className="h-4 w-4 ml-2"/> تم الصرف
            </Button>)}
        </div>
      </Card>

      <PhoneSheet open={phoneOpen} onOpenChange={setPhoneOpen} patient={{ ...row, patient_id: row.patient_id, patient_name: row.patient_name, phone: row.phone }} pharmacy={session?.unlocked ? { id: session.pharmacy.id, name: session.pharmacy.name } : undefined}/>
      <DispenseDialog open={dispOpen} onOpenChange={setDispOpen} patientId={row.patient_id} patientName={row.patient_name} cardNumber={row.insurance_card_number}/>
      <RemainingConfirmDialog open={remOpen} onOpenChange={setRemOpen} patientId={row.patient_id} defaultPharmacyId={session?.unlocked ? session.pharmacy.id : undefined}/>
    </>);
}
