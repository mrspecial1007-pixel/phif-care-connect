...
function normalizeLibyaIntl(raw) {
...
function generateMessage(row, pharmacyName) {
    let text = "";
...
export function PhoneSheet({ open, onOpenChange, patient, pharmacy, }) {
        ...
        <SheetContent side="bottom" className="rounded-t-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Textarea value={message} onChange={(e) => setMessage(e.target.value)} className="min-h-[150px] text-right" dir="rtl"/>
            </div>

            <div className="grid grid-cols-2 gap-2">
              {channel === "WhatsApp" ? (<Button onClick={openWhatsApp} className="bg-success hover:bg-success/90 text-white col-span-2 h-12">
                  <MessageSquare className="h-5 w-5 ml-2"/> فتح في WhatsApp
                </Button>) : (<Button onClick={openSMS} className="bg-info hover:bg-info/90 text-info-foreground col-span-2 h-12">
