...
import { saveSubscription, getSubscriptionSettings, updateSubscriptionSettings, deleteSubscription, sendTestNotification } from "@/lib/notifications/notifications.functions";
...
function NotificationSettingsCard() {
    ...
    const [subscriptionId, setSubscriptionId] = useState(typeof localStorage !== 'undefined' ? localStorage.getItem('push_subscription_id') : null);
    ...
    async function disableThisDevice() {
        if (!subscriptionId)
            return;
        if (!confirm("هل أنت متأكد من إلغاء الإشعارات لهذا الجهاز؟"))
            return;
        try {
    ...
    if (typeof Notification === 'undefined') {
        return (<Card className="p-4 bg-muted/50">
        <div className="flex items-center gap-3">
    ...
    return (<Card className="p-4 space-y-6">
      <div className="flex items-center justify-between border-b pb-2">
        ...
        {subscriptionId && (<Button variant="ghost" size="sm" className="text-destructive h-8" onClick={disableThisDevice}>
            <Trash2 className="h-4 w-4 ml-1"/>
            إلغاء هذا الجهاز
          </Button>)}
      </div>

      {!subscriptionId ? (<div className="space-y-4">
          ...
          {settings && (<div className="space-y-5">
              <div className="flex items-center justify-between">
                ...
                <Switch id="master-toggle" checked={settings.notifications_enabled} onCheckedChange={(val) => setSettings({ ...settings, notifications_enabled: val })}/>
              </div>

              <div className={`space-y-4 transition-opacity ${settings.notifications_enabled ? 'opacity-100' : 'opacity-40 pointer-events-none'}`}>
                <NotificationTypeRow label="ملخص الصباح" description="يومياً، يجمع كل المواعيد" enabled={settings.morning_summary_enabled} time={settings.morning_summary_time} onEnabledChange={(v) => setSettings({ ...settings, morning_summary_enabled: v })} onTimeChange={(v) => setSettings({ ...settings, morning_summary_time: v })}/>
                <NotificationTypeRow label="مستحقون اليوم" description="تنبيه بمجرد حلول موعد الصرف" enabled={settings.due_today_enabled} time={settings.due_today_time} onEnabledChange={(v) => setSettings({ ...settings, due_today_enabled: v })} onTimeChange={(v) => setSettings({ ...settings, due_today_time: v })}/>
                <NotificationTypeRow label="مستحقون غدًا" description="تنبيه مسبق للمستحقين غداً" enabled={settings.due_tomorrow_enabled} time={settings.due_tomorrow_time} onEnabledChange={(v) => setSettings({ ...settings, due_tomorrow_enabled: v })} onTimeChange={(v) => setSettings({ ...settings, due_tomorrow_time: v })}/>
                <NotificationTypeRow label="متأخر يوم أو يومين" description="تنبيه للمتأخرين فقط لأول يومين" enabled={settings.overdue_enabled} time={settings.overdue_time} onEnabledChange={(v) => setSettings({ ...settings, overdue_enabled: v })} onTimeChange={(v) => setSettings({ ...settings, overdue_time: v })}/>
                <NotificationTypeRow label="صرف جزئي" description="متابعة حالات الصرف غير المكتملة" enabled={settings.partial_enabled} time={settings.partial_time} onEnabledChange={(v) => setSettings({ ...settings, partial_enabled: v })} onTimeChange={(v) => setSettings({ ...settings, partial_time: v })}/>
                ...
                <div className="pt-4 border-t space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <Switch id="quiet-toggle" checked={settings.quiet_hours_enabled} onCheckedChange={(val) => setSettings({ ...settings, quiet_hours_enabled: val })}/>
                  </div>
                  
                  {settings.quiet_hours_enabled && (<div className="flex items-center gap-4 pr-6">
                      <div className="flex items-center gap-2 flex-1">
                        <span className="text-xs text-muted-foreground">من:</span>
                        <Input type="time" value={settings.quiet_hours_start.slice(0, 5)} onChange={(e) => setSettings({ ...settings, quiet_hours_start: e.target.value })} className="h-8 py-1 px-2"/>
                      </div>
                      <div className="flex items-center gap-2 flex-1">
                        <span className="text-xs text-muted-foreground">إلى:</span>
                        <Input type="time" value={settings.quiet_hours_end.slice(0, 5)} onChange={(e) => setSettings({ ...settings, quiet_hours_end: e.target.value })} className="h-8 py-1 px-2"/>
                      </div>
                    </div>)}
                </div>
              </div>
...
function NotificationTypeRow({ label, description, enabled, time, onEnabledChange, onTimeChange }) {
    return (<div className="flex items-start justify-between gap-4">
      <div className="flex-1">
        <div className="font-medium">{label}</div>
        <div className="text-xs text-muted-foreground">{description}</div>
        {enabled && (<div className="mt-1 flex items-center gap-1.5 text-[10px] text-primary/70 font-medium">
             — يومياً الساعة {time.slice(0, 5)}
          </div>)}
      </div>
      <div className="flex flex-col items-end gap-2">
        <Switch checked={enabled} onCheckedChange={onEnabledChange}/>
        {enabled && (<Input type="time" value={time.slice(0, 5)} onChange={(e) => onTimeChange(e.target.value)} className="h-7 w-20 text-[11px] p-1"/>)}
      </div>
    </div>);
}
function PharmacySettingsCard() {
    ...
    const [busy, setBusy] = useState(false);
    if (!session?.unlocked || !session.pharmacy)
        return null;
    const pharmacy = session.pharmacy;
    async function updatePharmacyInfo(e) {
        e.preventDefault();
        setBusy(true);
    ...
    return (<Card className="p-4 space-y-4">
      ...
      <form onSubmit={updatePharmacyInfo} className="space-y-4">
        <div className="space-y-2">
          ...
          <input name="name" defaultValue={pharmacy.name} className="w-full p-2 border rounded-md text-right" required dir="rtl"/>
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium">العنوان (يظهر في الرسائل)</label>
          <textarea name="address" defaultValue={pharmacy.address} className="w-full p-2 border rounded-md text-right min-h-[80px]" required dir="rtl"/>
        </div>
        <div className="space-y-2">
          <label className="text-sm font-medium">رقم الهاتف (اختياري)</label>
          <input name="phone" defaultValue={pharmacy.phone} className="w-full p-2 border rounded-md text-right" dir="ltr"/>
        </div>
