...
import { Building2, Pill, Pencil, FileSpreadsheet, Download, Phone, MessageSquare } from "lucide-react";
...
function ActivityPage() {
    ...
    const [filter, setFilter] = useState("today_all");
    const items = useMemo(() => {
        if (!data)
            return [];
        ...
        const auditItems = data.audit
            .filter((a) => a.action !== "record_dispensing" && a.action !== "record_dispensing_historical_append" && a.action !== "record_dispensing_recalc")
            .map((a) => ({ kind: "audit", at: a.created_at, audit: a }));
        const commItems = data.communication?.map((c) => ({
            kind: "comm",
            at: c.created_at,
            comm: c,
        })) || [];
        ...
        return all.filter((it) => {
            ...
            if (filter === "dispensing")
                return it.kind === "tx";
            if (filter === "edits")
                return it.kind === "audit" && it.audit.action?.includes("patient");
            if (filter === "imports")
                return it.kind === "audit" && it.audit.action === "import_excel";
    ...
    return (<div className="space-y-3">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {chips.map((c) => (<button key={c.k} onClick={() => setFilter(c.k)} className={`whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium border ${filter === c.k ? "bg-primary text-primary-foreground border-primary" : "bg-card text-muted-foreground"}`}>
            {c.label}
          </button>))}
      </div>

      {isLoading ? (<div className="text-center py-10 text-muted-foreground">جاري التحميل…</div>) : items.length === 0 ? (<div className="text-center py-10 text-muted-foreground">لا يوجد نشاط</div>) : (<div className="space-y-2">
          {items.map((it, i) => (<Card key={i} className="p-3">
              {it.kind === "tx" ? (<TxRow tx={it.tx}/>) : it.kind === "comm" ? (<CommRow comm={it.comm}/>) : (<AuditRow a={it.audit}/>)}
            </Card>))}
        </div>)}
    </div>);
}
function TxRow({ tx }) {
    ...
    const kind = tx.transaction_type === "Completed" ? { label: "صرف كامل", cls: "text-success" } :
...
function CommRow({ comm }) {
    ...
    return (<div className="flex items-start gap-3">
      ...
      {isCall ? (<Phone className="h-4 w-4 mt-0.5 text-primary shrink-0"/>) : (<MessageSquare className="h-4 w-4 mt-0.5 text-success shrink-0"/>)}
      <div className="flex-1 min-w-0">
