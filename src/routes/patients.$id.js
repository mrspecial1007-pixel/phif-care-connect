...
import { ArrowRight, Pill, Building2, Share2, AlertTriangle, Copy, Pencil, Plus, CheckCircle2, Clock, MapPin, Phone, CreditCard, } from "lucide-react";
    ...
    return (<div className="space-y-4">
      ...
      <Card className="p-4 space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {status?.is_shared && (<Badge variant="outline">
                  <Share2 className="h-3 w-3 ml-1"/> مشترك
                </Badge>)}
              {patient.review_status === "needs_review" && (<Badge className="bg-warning text-warning-foreground border-0">
        ...
        <div className="space-y-2 text-sm">
          <InfoRow icon={<CreditCard className="h-4 w-4"/>} label="رقم البطاقة" value={patient.insurance_card_number} onCopy={() => patient.insurance_card_number &&
            copy(patient.insurance_card_number, "رقم البطاقة")} onAdd={() => {
            setEditFocus("insurance_card_number");
            setEditOpen(true);
        }} emptyLabel="لا يوجد رقم بطاقة" addLabel="إضافة رقم البطاقة"/>
          <InfoRow icon={<Phone className="h-4 w-4"/>} label="رقم الهاتف" value={patient.phone} onAction={() => patient.phone && setPhoneOpen(true)} actionLabel="إجراءات الاتصال" onAdd={() => {
            setEditFocus("phone");
            setEditOpen(true);
        }} emptyLabel="لا يوجد رقم هاتف" addLabel="إضافة رقم الهاتف"/>
          <InfoRow icon={<MapPin className="h-4 w-4"/>} label="العنوان" value={patient.address} onAdd={() => {
            setEditFocus("address");
            setEditOpen(true);
        }} emptyLabel="لا يوجد عنوان" addLabel="إضافة العنوان"/>
        </div>
      </Card>
      ...
      <Card className="p-4 space-y-2">
        ...
        <div className="grid grid-cols-2 gap-3 text-sm">
          ...
          <Stat label="الأيام" value={status?.remaining_days == null
            ? "—"
            : status.remaining_days >= 0
                ? `متبقي ${status.remaining_days} يوم`
                : `متأخر ${Math.abs(status.remaining_days)} يوم`} tone={status?.remaining_days == null
            ? undefined
            : status.remaining_days < 0
                ? "danger"
                : status.remaining_days <= 3
                    ? "warning"
                    : "muted"}/>
          <Stat label="آخر صيدلية" value={status?.last_pharmacy_name ?? "—"} icon={<Building2 className="h-3.5 w-3.5"/>}/>
        </div>

        {isPartial ? (<Button className="w-full h-14 text-base bg-info text-info-foreground hover:bg-info/90" onClick={() => setRemainingConfirmOpen(true)}>
            <Clock className="h-5 w-5 ml-2"/> صرف متبقي
          </Button>) : (<Button className="w-full h-14 text-base" onClick={() => setDispenseOpen(true)}>
      ...
      <Card className="p-4">
        ...
        <div className="space-y-2">
          {(history ?? []).map((h) => (<div key={h.id} className="border-b pb-2 last:border-0">
              <div className="flex items-center gap-2">
                <div className="text-xs w-24 text-muted-foreground shrink-0">
                  {new Date(h.dispensing_date).toLocaleDateString("en-GB")}
                </div>
                <Badge className={`border-0 ${h.transaction_type === "Partial"
              ...
              {h.notes && (<div className="text-xs text-muted-foreground mt-1 pr-24">{h.notes}</div>)}
            </div>))}
          {(history ?? []).length === 0 && (<div className="text-sm text-muted-foreground text-center py-4">لا توجد سجلات</div>)}
        </div>
      </Card>
      ...
      <RemainingConfirmDialog open={remainingConfirmOpen} onOpenChange={setRemainingConfirmOpen} patientId={patient.id} defaultPharmacyId={session?.unlocked ? session.pharmacy.id : undefined}/>

      {patient?.phone && (<PhoneSheet open={phoneOpen} onOpenChange={setPhoneOpen} patient={{
                patient_id: patient.id,
                patient_name: patient.patient_name,
                phone: patient.phone,
                remaining_days: status?.remaining_days,
                current_cycle_status: status?.current_cycle_status,
...
function Stat({ label, value, tone, icon, }) {
    const toneCls = tone === "danger"
        ...
        : tone === "warning"
            ? "text-warning"
            : "text-foreground";
    return (<div className="rounded-lg border p-2">
...
function InfoRow({ icon, label, value, onCopy, onAction, actionLabel, onAdd, emptyLabel, addLabel, }) {
    if (value) {
        return (<div className="flex items-center gap-2 rounded-lg border p-2">
        <span className="text-muted-foreground">{icon}</span>
        <button onClick={onAction} disabled={!onAction} className="flex-1 min-w-0 text-right disabled:cursor-default">
          <div className="text-[11px] text-muted-foreground">{label}</div>
          <div className={`text-sm font-medium truncate ${onAction ? "text-cyan-700 dark:text-cyan-400" : ""}`} dir="ltr">
            {value}
          </div>
        </button>
        {onCopy && (<Button size="icon" variant="ghost" onClick={onCopy} aria-label={`نسخ ${label}`}>
            <Copy className="h-4 w-4"/>
          </Button>)}
        {actionLabel && onAction && (<Button size="icon" variant="ghost" onClick={onAction} aria-label={actionLabel}>
            <Phone className="h-4 w-4"/>
          </Button>)}
      </div>);
    }
    return (<div className="flex items-center gap-2 rounded-lg border border-dashed p-2">
      <span className="text-muted-foreground">{icon}</span>
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <Input value={card} onChange={(e) => setCard(e.target.value)} maxLength={60} dir="ltr" inputMode="numeric" autoFocus={focusField === "insurance_card_number"}/>
          </div>
          <div>
            <Label>رقم الهاتف</Label>
            <Input value={phone} onChange={(e) => setPhone(e.target.value)} maxLength={40} dir="ltr" inputMode="tel" autoFocus={focusField === "phone"}/>
          </div>
          <div>
            <Label>العنوان</Label>
            <Textarea value={address} onChange={(e) => setAddress(e.target.value)} maxLength={500} autoFocus={focusField === "address"}/>
          </div>
        </div>
