...
export async function processScheduledNotifications() {
    ...
    if (!beneficiaries)
        return;
    const dueToday = beneficiaries.filter((b) => b.remaining_days === 0).length;
    const dueTomorrow = beneficiaries.filter((b) => b.remaining_days === 1).length;
    const overdue12 = beneficiaries.filter((b) => b.remaining_days === -1 || b.remaining_days === -2).length;
    ...
    const partialCount = beneficiaries.filter((b) => (b.active_tracks?.length || 0) > 1).length;
    for (const sub of subscriptions) {
        // Check quiet hours
        if (sub.quiet_hours_enabled) {
            const start = sub.quiet_hours_start;
            const end = sub.quiet_hours_end;
            if (isTimeBetween(currentTime, start, end))
                continue;
        }
...
async function processType(sub, type, currentTime, today, payload) {
    ...
    if (scheduledTime !== currentTime.slice(0, 5))
        return;
