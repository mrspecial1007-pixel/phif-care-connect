-- 1. Add national_id to patients if not present (it was already in types but let's be sure)
-- types.ts shows it already exists.

-- 2. Add soft-delete fields to patients
...
-- 3. Add cancellation fields to dispensing_transactions
...
-- 4. Update v_patient_status view to ignore cancelled transactions and archived patients
-- We need to drop and recreate it because it might depend on the table structure.
...
-- Ensure RLS is enabled and proper grants exist for new columns
