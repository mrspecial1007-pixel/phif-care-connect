-- Drop and recreate v_patient_status to include track status aggregation
...
CREATE OR REPLACE VIEW public.v_patient_status AS
 WITH track_stats AS (
         SELECT dispensing_due_tracks.patient_id,
            ...
            -- Check if any active track was born from a partial dispense
            bool_or(dispensing_due_tracks.status = 'Partial') as is_partial,
            jsonb_agg(
                jsonb_build_object(
 ...
 SELECT p.id AS patient_id,
    ...
    CASE 
      WHEN ts.is_partial OR ts.active_tracks_count > 1 THEN 'Partial'::text
      WHEN ts.active_tracks_count = 1 THEN 'Waiting'::text
      ELSE 'Completed'::text
