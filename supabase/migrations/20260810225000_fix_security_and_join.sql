-- Fix the security definer view lint error by setting security_invoker = true
-- Also fix the ambiguous relationship hint for the view if needed, 
-- but the error reported was in queries.ts TypeScript types.

DROP VIEW IF EXISTS public.v_patient_status;

CREATE OR REPLACE VIEW public.v_patient_status 
